 ...
 * @author Oscar Lao
 */
public class Activation_Generalized_Gaussian extends Activation_Generalized{
     ...
     * @param c
     */
    public Activation_Generalized_Gaussian(VDouble k, VDouble s, VDouble c) {
        super(k);
        this.s = s;
        this.c = c;
    }
     ...
     */
    public Activation_Generalized_Gaussian() {
        super();
    ...
    public double activate(double v) {
        ...
        double a = 2*Math.pow(s.getI().getD(),2);
//        a = 0.01*a/(1+0.01*Math.abs(a));
        double act = -Math.pow(v + c.getI().getD(),2)/a;
//        act = 0.001*(0.5*0.1*act/(1+0.1*Math.abs(act))+0.5);
        return getK().getI().getD() *Math.exp(act);
...
//        return act;        
    ...
    public Activation_Function copy() {
        return new Activation_Generalized_Gaussian((VDouble) getK().copy(), (VDouble) s.copy(), (VDouble) c.copy());
    }
