    ...
    private Individual generateNewIndividual() throws ExceptionFragment, ExceptionIndividual, ExceptionGenome {
        ...
        for (int g = 0; g < history.getPg().size(); g++) {
            for (int l = 0; l < 2; l++) {
                ...
                Fragment f = new Fragment(0, (int)history.getPg().get(g).getMutationList().getFragment().length());
                f.add(seq);
                ...
                Fragment fs = new Fragment(0, (int)history.getPg().get(g).getMutationList().getFragment().length());
                fs.add(seq);
