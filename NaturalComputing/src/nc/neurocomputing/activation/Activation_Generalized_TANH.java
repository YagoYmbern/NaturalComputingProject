 ...
 * @author Oscar Lao
 */
public class Activation_Generalized_TANH extends Activation_Generalized{
     ...
     * @param c
     */
    public Activation_Generalized_TANH(VDouble k, VDouble s, VDouble c) {
        super(k);
        this.s = s;
        this.c = c;
    }
     ...
     */
    public Activation_Generalized_TANH() {
        super();
...
//        act = 0.001*(0.5*0.1*act/(1+0.1*Math.abs(act))+0.5);
        return getK().getI().getD() *(Math.exp(act)-Math.exp(-act))/(Math.exp(act)+Math.exp(-act));
...
//        return act;        
    ...
    public Activation_Function copy() {
        return new Activation_Generalized_TANH((VDouble) getK().copy(), (VDouble) s.copy(), (VDouble) c.copy());
    }
